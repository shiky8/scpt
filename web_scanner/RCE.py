...
class RCE_Scan():
    ...
    def is_vulnerable(self,response):
        ...
        is RCE vulnerable from its `response`"""
        errors = {
            # id
            "uid=",
            "gid=",
    ...
    def scan_RCE(self, url):
        ...
        RCE_payloads = open("RCE_Payloads.txt","r")
        # test on URL
        for c in RCE_payloads:
            ...
            try:
                ...
                if self.is_vulnerable(res):
                    # RCE detected on the URL itself,
                    # no need to preceed for extracting forms and submitting them
                    print("[+] RCE vulnerability detected, link:", new_url)
                    with open(dirk + url.replace('http://', '').replace('https://', '').replace('/',
                                                                                                '') + "_WEB_RCE.json",
                              "a") as dop:
                        dop.write("[+] RCE vulnerability detected, link:" + new_url)
                        dop.write("\n")
                        dop.close()
                    return
            except:
        ...
        for form in forms:
            ...
            for c in RCE_payloads:
                ...
                if self.is_vulnerable(res):
                    print("[+] RCE vulnerability detected, link:", url)
                    print("[+] Form:")
                    pprint(form_details)
                    print("Payload:", c)
                    with open(dirk + url.replace('http://', '').replace('https://', '').replace('/','') + "_WEB_RCE.json","a") as dop:
                        dop.write("[+] RCE vulnerability detected, link:"+ url)
                        dop.write("\n")
...
class GUI(QtCore.QThread):
        ...
        def run(self) -> None:
            RCE = RCE_Scan(self.cookieop)
            dirk: str = str(sp.getoutput('pwd'))
            if ("/web_scanner" not in dirk):
                dirk += "/web_scanner"
            dirk = dirk.replace('/web_scanner', '/Reports/WEB_bugs/')
            RCE_payloads = open("/home/shiky/PycharmProjects/scpt/web_scanner/RCE_Payloads.txt", "r")
            # test on URL
            for c in RCE_payloads:
                # add quote/double quote character to the URL
                ...
                try:
                    req1 = Request(new_url, headers=header)
                    req1.add_header('Cookie', RCE.my_cook)
                    res: str = urlopen(req1).read().decode()
                    # soup = bs(req, "html.parser")  #
                    # res = s.get(new_url)#
                    if RCE.is_vulnerable(res):
                        # RCE detected on the URL itself,
                        # no need to preceed for extracting forms and submitting them
                        print("[+] RCE vulnerability detected, link:", new_url)
                        self.Gui_Date_output.emit(str("[+] RCE vulnerability detected, link :"+ new_url))
                        time.sleep(1)
                        QtCore.QCoreApplication.processEvents()
                    ...
                    pass
            # test on HTML forms
            forms = RCE.get_all_forms(self.url)
            ...
            RCE_payloads = open("/home/shiky/PycharmProjects/scpt/web_scanner/RCE_Payloads.txt", "r")
            for form in forms:
                form_details = RCE.get_form_details(form)
                for c in RCE_payloads:
                    # the data body we want to submit
                    data = {}
                    for input_tag in form_details["inputs"]:
                    ...
                    if RCE.is_vulnerable(res):
                        ...
                        pprint(form_details)
                        print("Payload:", c)
                        self.Gui_Date_output.emit(str("[+] RCE vulnerability detected, link: "+ url))
